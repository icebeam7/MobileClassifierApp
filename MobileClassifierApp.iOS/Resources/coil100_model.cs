...
namespace MobileClassifierApp.iOS {
	/// <summary>
	/// Model Prediction Input Type
	/// </summary>
	public class coil100_modelInput : NSObject, IMLFeatureProvider
	{
		...
		public coil100_modelInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class coil100_modelOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public coil100_modelOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class coil100_model : NSObject
	{
		readonly MLModel model;

		static NSUrl GetModelUrl ()
		{
			return NSBundle.MainBundle.GetUrlForResource ("coil100_model", "mlmodelc");
		}

		public coil100_model ()
		{
			NSError err;
		...
		coil100_model (MLModel model)
		{
			this.model = model;
		}

		public static coil100_model Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new coil100_model (model);
		}

		public static coil100_model Create (MLModelConfiguration configuration, out NSError error)
		{
		...
		public static coil100_model Create (NSUrl url, MLModelConfiguration configuration, out NSError error)
		{
			if (url == null)
				throw new ArgumentNullException (nameof (url));
		...
		/// </summary>
		/// <param name="input">an instance of coil100_modelInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public coil100_modelOutput GetPrediction (coil100_modelInput input, out NSError error)
		{
			if (input == null)
			...
			return new coil100_modelOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public coil100_modelOutput GetPrediction (coil100_modelInput input, MLPredictionOptions options, out NSError error)
		{
			if (input == null)
				throw new ArgumentNullException (nameof (input));
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public coil100_modelOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new coil100_modelInput (data);
		...
		/// <param name="options">prediction options</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public coil100_modelOutput GetPrediction (CVPixelBuffer data, MLPredictionOptions options, out NSError error)
		{
			var input = new coil100_modelInput (data);
